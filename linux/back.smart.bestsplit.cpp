...
pthread_mutex_t indexMutex = PTHREAD_MUTEX_INITIALIZER, threadMutex = PTHREAD_MUTEX_INITIALIZER;
...
void bestsplit(node *n, params *p, string response, int & numleft, int & numright)
{
    ...
    for (int varIdx = 0; varIdx < colCount; varIdx++) {
        ...
        if (p->delayed && improve > 0) {
            bestLeftSS = DBL_MAX, bestRightSS = DBL_MAX;
            int varIdx2 = 0;
            int indx = 0;
            ...
            while(varIdx2 < colCount) {
                ...
                for(int i = 0; i < threads_ready; i++) {
                    ...
                    thread_params param;
                    param.L1 = deepCopyData(L1, numLeft, colCount);
                    param.L2 = deepCopyData(L2, numRight, colCount);
                    param.varIdx2 = varIdx2;
                    param.p = p;
                    param.numLeft = numLeft;
                    param.numRight = numRight;
                    param.colCount = colCount;
                    param.respCol = respCol;
                    param.split_func = split_func;
        ...
        if (improve > 0 && trunc(1000000.*thisSS) < trunc(1000000.*bestSS)) { 
            bestSS = thisSS;
...
void *delayedThread(void *param_cast) {
    struct thread_params *param = (thread_params*)param_cast;

    params *p = param->p;
    int numLeft = param->numLeft, numRight = param->numRight, varIdx2 = param->varIdx2;
    int respCol = param->respCol, colCount = param->colCount;
    float thisSSLeft = 0, thisSSRight = 0;

    float *x = getExplanatoryDataCol(p->response, p->headers, param->L1, numLeft, varIdx2);
...
/*    int whereL, directionL, whereR, directionR;
    ...
    // left data
    (*param->split_func)(x, y, p, whereL, directionL, splitPointL, improveL, numLeft);
    free1DData(y);
    free1DData(x);

    int L3Size = 0, L4Size = 0;
    getSplitCounts(param->L1, varIdx2, splitPointL, directionL, numLeft, L3Size, L4Size);
    ...
    splitData(directionL, splitPointL, varIdx2, colCount, numLeft, L3, L4, param->L1);
    double l3SS, l4SS;
    ...
    // right data
    x = getExplanatoryDataCol(p->response, p->headers, param->L2, numRight, varIdx2);
    y = getResponseData(p->response, p->headers, param->L2, numRight);
    mergeSort(x, y, 0, numRight-1, varIdx2, colCount, respCol);

    (*param->split_func)(x, y, p, whereR, directionR, splitPointR, improveR, numRight);
    free1DData(y);
    free1DData(x);

    int L5Size = 0, L6Size = 0;
    getSplitCounts(param->L2, varIdx2, splitPointR, directionR, numRight, L5Size, L6Size);
    ...
    splitData(directionR, splitPointR, varIdx2, colCount, numRight, L5, L6, param->L2);
    double l5SS, l6SS;
